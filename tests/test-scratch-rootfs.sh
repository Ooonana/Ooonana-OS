...
[[ -L "$rootfs/bin/wget" ]] || fail "missing wget applet"
[[ -L "$rootfs/bin/adduser" ]] || fail "missing adduser applet"
[[ -L "$rootfs/bin/awk" ]] || fail "missing awk applet"
[[ -L "$rootfs/bin/clear" ]] || fail "missing clear applet"
[[ -L "$rootfs/bin/ifconfig" ]] || fail "missing ifconfig applet"
[[ -L "$rootfs/bin/ip" ]] || fail "missing ip applet"
[[ -L "$rootfs/bin/passwd" ]] || fail "missing passwd applet"
[[ -L "$rootfs/bin/route" ]] || fail "missing route applet"
[[ -L "$rootfs/bin/udhcpc" ]] || fail "missing udhcpc applet"
...
[[ -x "$rootfs/usr/bin/ooonana-setup" ]] || fail "missing ooonana setup"
...
[[ -f "$rootfs/etc/profile" ]] || fail "missing profile"
[[ -f "$rootfs/etc/profile.d/ooonana-shell.sh" ]] || fail "missing shell helpers"
[[ -f "$rootfs/etc/neofetch/config.conf" ]] || fail "missing neofetch config"
[[ -d "$rootfs/dev/pts" ]] || fail "missing devpts mountpoint"
...
if grep -q $'\r' "$ROOT/packages/ooonana/var/lib/ooonana/packages/installed/ooonana-core.pkg"; then
  fail "ooonana-core metadata contains CRLF"
fi
assert_contains "$rcs" "OOONANA_INSTALL_OK"
assert_contains "$rcs" "Ooonana installer"
assert_contains "$rcs" "Target disk:"
assert_contains "$rcs" "Source image:"
assert_contains "$rcs" "Writing image to target"
...
assert_contains "$rcs" "cmdline_value()"
assert_contains "$rcs" "cmdline_value 'ooonana.install.target'"
...
assert_contains "$rcs" 'exec /bin/sh -l <"$console_device" >"$console_device" 2>&1'

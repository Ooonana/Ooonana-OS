...
assert_contains "$(<"$AI_DESKTOP_APP")" 'OOONANA_AI_APP_NO_X:-0}'
assert_contains "$(<"$AI_DESKTOP_APP")" 'OOONANA_AI_APP_TERMINAL:-0}'
...
assert_contains "$app_gui_dry" "AI workbench layout"
assert_contains "$app_gui_dry" "workbench panes: Chat ActionRail Context Permissions Logs"
assert_contains "$app_gui_dry" "chat-first controls: prompt transcript model provider tools save clear"
...
assert_contains "$(<"$AI_DESKTOP_APP")" "show_workbench"
assert_contains "$(<"$AI_DESKTOP_APP")" "choose_workbench_action"
assert_contains "$(<"$AI_DESKTOP_APP")" "context_gui"
...
assert_contains "$(<"$config")" "OOONANA_OPENVINO_MODEL=qwen3.5-9b-int4-ov"
...
assert_not_contains "$fallback_status" "state_dir:"

...
assert_contains "$config" 'bindsym --whole-window $mod+button1 move'
assert_contains "$config" 'bindsym --whole-window $mod+button3 resize'
assert_contains "$config" 'polybar -c /etc/ooonana/polybar.ini ooonana'
assert_contains "$config" 'polybar -c /etc/ooonana/polybar.ini ooonana-dock'
assert_contains "$config" 'rofi -show drun -theme /etc/ooonana/rofi.rasi'
assert_contains "$config" 'picom --backend glx --config /etc/ooonana/picom.conf'
assert_contains "$config" 'picom --backend xrender --config /etc/ooonana/picom.conf'
...
assert_contains "$config" 'bindsym $mod+m move scratchpad'
...
assert_contains "$config" 'bindsym $mod+Shift+b exec ooonana-rofi-bluetooth'
assert_contains "$config" 'bindsym $mod+Shift+n exec ooonana-rofi-wifi'
...
assert_contains "$config" 'bindsym XF86TouchpadToggle exec ooonana-touchpad toggle'
assert_contains "$config" 'bindsym XF86TouchpadOn exec ooonana-touchpad on'
assert_contains "$config" 'bindsym XF86TouchpadOff exec ooonana-touchpad off'

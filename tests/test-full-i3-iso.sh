...
assert_contains "$script_src" "ooonana_boot_hint"
assert_not_contains "$script_src" "[#####-----]"
verifier_src="$(<"$ROOT/scripts/verify-rufus-iso.sh")"
assert_contains "$verifier_src" "function ooonana_boot_hint"
assert_not_contains "$verifier_src" "ooonana_progress_bar"
...
assert_contains "$normal_cfg" "export color_normal"
assert_contains "$normal_cfg" "Arrows choose a mode. Enter boots."
grub_logo="$(<"$tmp/build/full-i3-iso-tree/boot/grub/ooonana-logo.txt")"
assert_contains "$grub_logo" "__________________"
assert_contains "$grub_logo" '|     \______/     |'
expected_grub_logo="$(while IFS= read -r line || [[ -n "$line" ]]; do printf '%26s%s\n' '' "$line"; done < "$ROOT/packages/ooonana/usr/share/ooonana/grub-logo.txt")"
[[ "$grub_logo" == "$expected_grub_logo" ]] || fail "GRUB logo block alignment changed"
...
assert_not_contains "$normal_cfg" "750XGK"
assert_contains "$normal_cfg" "submenu 'Audio compatibility'"
assert_contains "$normal_cfg" "Live - force Intel SOF audio"
assert_contains "$normal_cfg" "Persistent USB - force Intel SOF audio"
assert_contains "$normal_cfg" "Live - force legacy Intel HDA audio"
assert_contains "$normal_cfg" "Persistent USB - force legacy Intel HDA audio"
assert_contains "$normal_cfg" "snd_intel_dspcfg.dsp_driver=1"
...
install_line="$(grep -n -m1 "^menuentry 'Install Ooonana OS Full i3'" <<<"$normal_cfg" | cut -d: -f1)"
audio_line="$(grep -n -m1 "^submenu 'Audio compatibility'" <<<"$normal_cfg" | cut -d: -f1)"
[[ -n "$install_line" && -n "$audio_line" && "$install_line" -lt "$audio_line" ]] ||
  fail "audio submenu must follow installer entries so smoke default 2 stays installer"
...
default_live_entry="$(awk '/^menuentry '\''Ooonana OS Full i3 Live'\''/ { show=1 } show { print } show && /^}/ { exit }' <<<"$live_smoke_cfg")"
assert_contains "$default_live_entry" "ooonana.smoke=1 ooonana.gui-smoke=1"
assert_not_contains "$default_live_entry" "ooonana.install=1"

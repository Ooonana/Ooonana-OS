...
if [ "$USE_ISO_RUNTIME" -eq 0 ]; then
  mkdir -p "$WORK/patch/smoke-root/usr/bin" "$WORK/patch/smoke-root/etc/sudoers.d"
...
for command in dbus-daemon NetworkManager nmcli bluetoothd bluetoothctl doas sudo su aplay; do

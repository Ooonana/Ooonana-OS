...
version="$("$CLI" version)"
[[ "$version" == "ooonana 0.9.1" ]] || fail "bad version: $version"

sh_version="$(sh "$CLI" version)"
[[ "$sh_version" == "ooonana 0.9.1" ]] || fail "bad sh version: $sh_version"
...
[[ "$pkg_help" == *"ooonana 0.9.1"* ]] || fail "help missing version header"

...
version="$("$CLI" version)"
[[ "$version" == "ooonana 0.8.28" ]] || fail "bad version: $version"

sh_version="$(sh "$CLI" version)"
[[ "$sh_version" == "ooonana 0.8.28" ]] || fail "bad sh version: $sh_version"
...
[[ "$pkg_help" == *"ooonana 0.8.28"* ]] || fail "help missing version header"

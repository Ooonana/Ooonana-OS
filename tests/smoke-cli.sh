...
version="$("$CLI" version)"
[[ "$version" == "ooonana 0.8.2" ]] || fail "bad version: $version"

sh_version="$(sh "$CLI" version)"
[[ "$sh_version" == "ooonana 0.8.2" ]] || fail "bad sh version: $sh_version"
...
[[ "$pkg_help" == *"ooonana 0.8.2"* ]] || fail "help missing version header"
...
[[ "$game_quit" == *" _________ "* ]] || fail "oonana game did not draw compact logo ball"
[[ "$game_quit" == *"<   \\_/   >"* ]] || fail "oonana game did not draw straight logo arms"

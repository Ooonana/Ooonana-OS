...
mkdir -p "$tmp/rootfs/lib/firmware/intel" "$tmp/rootfs/lib/firmware/rtl_bt" "$tmp/rootfs/lib/firmware/rtw89" \
  "$tmp/rootfs/lib/firmware/i915" "$tmp/rootfs/lib/firmware/amdgpu" "$tmp/rootfs/lib/firmware/radeon"
...
printf 'i915\n' > "$tmp/rootfs/lib/firmware/i915/adlp_dmc.bin"
printf 'amdgpu\n' > "$tmp/rootfs/lib/firmware/amdgpu/test.bin"
printf 'radeon\n' > "$tmp/rootfs/lib/firmware/radeon/test.bin"
...
assert_contains "$script_src" "i915/*"
assert_contains "$script_src" "amdgpu/*"
assert_contains "$script_src" "radeon/*"
...
assert_contains "$kernel_fragment" "CONFIG_FUSE_FS=y"
assert_contains "$kernel_fragment" "CONFIG_CUSE=y"

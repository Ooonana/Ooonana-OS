...
assert_contains "$attributes" "*.desktop text eol=lf"
assert_contains "$attributes" "packages/ooonana/usr/bin/* text eol=lf"
assert_contains "$attributes" "packages/*/rootfs/usr/bin/* text eol=lf"
...
assert_contains "$script_src" "no route"
assert_contains "$script_src" "sync 2>/dev/null || true"
...
assert_contains "$music_helper" "controls_app.py"
media_control="$(<"$rootfs/usr/bin/ooonana-media-control")"
assert_contains "$media_control" 'audio_output {'
assert_contains "$media_control" 'type "pulse"'
assert_contains "$media_control" "mpc update --wait"
assert_contains "$media_control" "play-pause"
media_status="$(<"$rootfs/usr/bin/ooonana-media-status")"
...
assert_contains "$audio_status" "no route"
...
no_backlight="$(bash -c 'brightnessctl() { return 1; }; . "$1"' bash "$rootfs/usr/bin/ooonana-brightness-status")"
[[ "$no_backlight" == ' --' ]] || fail "missing backlight shown as zero brightness"
backlight_ready="$(bash -c 'brightnessctl() { printf "backlight,intel,100,55%%\n"; }; . "$1"' bash "$rootfs/usr/bin/ooonana-brightness-status")"
[[ "$backlight_ready" == ' 55%' ]] || fail "real backlight level not shown"
...
assert_contains "$polybar_cfg" "click-middle = pactl set-sink-mute"
assert_contains "$polybar_cfg" "click-right = pavucontrol"
...
assert_contains "$polybar_cfg" "click-middle = brightnessctl set 50%"
assert_contains "$polybar_cfg" "[module/power]"
assert_contains "$polybar_cfg" "content = "
assert_contains "$polybar_cfg" "click-left = ooonana-power-menu"
assert_contains "$polybar_cfg" "click-right = i3lock"

...
assert_contains "$install_wizard" "Step 1/6: Target disk"
assert_contains "$install_wizard" "Step 2/6: User account"
assert_contains "$install_wizard" "Step 3/6: Hostname"
assert_contains "$install_wizard" "Step 4/6: Theme"
assert_contains "$install_wizard" "Step 5/6: Confirm install"
assert_contains "$install_wizard" "Step 6/6: Installing"
...
wizard_dry="$("$rootfs/usr/bin/ooonana-install-wizard" --dry-run --target /dev/vdb --source / --user ryan --hostname ooonana-lab --theme dark)"
assert_contains "$wizard_dry" "Step 1/6 target: /dev/vdb"
assert_contains "$wizard_dry" "Step 2/6 user: ryan"
assert_contains "$wizard_dry" "Step 3/6 hostname: ooonana-lab"
assert_contains "$wizard_dry" "Step 4/6 theme: dark"
assert_contains "$wizard_dry" "Step 5/6 confirm: INSTALL"
assert_contains "$wizard_dry" "Step 6/6 reboot: optional"
assert_contains "$wizard_dry" "/usr/sbin/ooonana-install --target /dev/vdb --source / --hostname ooonana-lab --user ryan --theme dark --yes"

...
assert_contains "$install_wizard" "Step 1/4: Target disk"
assert_contains "$install_wizard" "Step 2/4: Source root"
assert_contains "$install_wizard" "Step 3/4: Confirm install"
assert_contains "$install_wizard" "Step 4/4: Installing"
...
wizard_dry="$("$rootfs/usr/bin/ooonana-install-wizard" --dry-run --target /dev/vdb --source /)"
assert_contains "$wizard_dry" "Step 1/4 target: /dev/vdb"
assert_contains "$wizard_dry" "Step 2/4 source: /"
assert_contains "$wizard_dry" "/usr/sbin/ooonana-install --target /dev/vdb --source / --yes"

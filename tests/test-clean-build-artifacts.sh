...
assert_contains "$help" "--keep-repo"
...
bash "$SCRIPT" --work-dir "$work" --keep-source --keep-repo --yes >/dev/null
...
[[ -e "$work/full-i3-repo" ]] || fail "keep-repo removed installable packages"
...
[[ -e "$work/ooonana-kernel" ]] || fail "keep-source removed cached boot kernel"
[[ -e "$work/kernel-build" ]] || fail "keep-source removed kernel build cache"
...
[[ ! -e "$work/ooonana-kernel" ]] || fail "full clean retained cached boot kernel"
[[ ! -e "$work/kernel-build" ]] || fail "full clean retained kernel build cache"
[[ ! -e "$work/full-i3-repo" ]] || fail "full clean retained package repository"

...
write_kernel_env() {
  ...
  [[ "$kernel_version" =~ ^[0-9]+\.[0-9]+\.[0-9]+[-._+a-zA-Z0-9]*$ ]] ||
    ooonana_die "invalid kernel version: $kernel_version"
...
verify_required_config() {
  local symbol
  local required=(
    BLK_DEV_INITRD DEVTMPFS EXT4_FS ISO9660_FS OVERLAY_FS FUSE_FS CUSE
    SWAP ZRAM ZRAM_BACKEND_LZ4 ZRAM_DEF_COMP_LZ4
    ...
    SND_HDA_INTEL SND_SOC_SOF_HDA_LINK SND_SOC_SOF_HDA_GENERIC
    SND_SOC_SOF_HDA_AUDIO_CODEC SND_SOC_HDAC_HDA
    SND_SOC_INTEL_SKL_HDA_DSP_GENERIC_MACH
    SND_SOC_SOF_METEORLAKE SND_SOC_SOF_INTEL_MTL
    SND_SOC_SOF_INTEL_SOUNDWIRE_LINK_BASELINE SND_SOC_SOF_IPC4
